...
export class CollabCardComponent implements OnInit {
  ...
  async makeTable() {
    if (this.alreadyBuilt) {
      ...
      this.xAxisReq = this.getAllRequired();
   ...
   */
  checkIfKnown(userName: string, listOfRequired: string[]) {
    ...
    this.userService.getMemberdetails(userName).subscribe((data) => {
      knownByUser = data["user"].skills.concat(data["user"].classes);
   ...
   */
  isUserOwner() {
    if (this.collabData.owner === this.userData["email"]) {
      this.isOwner = true;
    } else {
      this.isOwner = false;
    }
   ...
   */
  joinCollab() {
    this.collabService.joinCollab(this.collabData.id).subscribe((res) => {
      if (res["success"]) {
        this.partOf = true;
        this.getUpdatedData();
      } else {
   ...
   */
  leaveCollab() {
    this.collabService.leaveCollab(this.collabData.id).subscribe((res) => {
      if (res["success"] == true) {
        this.partOf = false;
        this.getUpdatedData();
      } else {
   ...
   */
  deleteCollab() {
    this.collabService.deleteCollab(this.collabData["id"]).subscribe((res) => {
      if (res) {
   ...
   */
  editCollab() {
    this.router.navigate(["/home/editcollab/", this.collabData.id]);
  }
   ...
   */
  getUpdatedData() {
    this.collabService.getSingleCollab(this.collabData.id).subscribe((res) => {
